-- ============================================================================
-- Gen 600: Hybrid Feature Sweep — UP + Intra Runup (No Gate) SHORT Template
--
-- PURPOSE: Test hybrid feature pairs (bar-level x lookback/intra) with
-- 3 barrier profiles per query via CROSS JOIN.
-- SHORT mirror of gen600_exh_l_ng_template.sql: barrier math flipped.
-- UP bar with intra_mru eligible, NO p75 gate — features replace the gate.
--
-- BASE PATTERN: Single UP bar + intra_max_runup IS NOT NULL → SHORT
-- SHORT ID: exh_s_ng
--
...
--   __CONFIG_ID__        — Config identifier (e.g., exh_l_ng__ofi_gt_p50__lookback_hurst_lt_p50)
--
