-- ============================================================================
-- Gen 720: Walk-Forward Barrier Optimization — exh Formation (SHORT)
--
...
--
-- FORMATION: Single UP bar + intra_max_runup > p75 (exhaustion)
-- DIRECTION: SHORT (sell on next-bar open after signal)
-- STRATEGY: B (reverse time-decay — tight SL first, then loosen)
...
formation_signals AS (
    ...
    WHERE dir_0 = 1
      AND intra_mru_0 IS NOT NULL
      AND intra_mru_0 > mru_p75_prior
      AND rn > 1000
      AND mru_p75_prior IS NOT NULL
...
barrier_scan AS (
    SELECT
        ...
        -- Strategy B: REVERSE time-decay — tight SL first, then loosen
        -- Segment 1: bars 1..phase1_bars with TIGHT SL (cut losers fast)
        arrayFirstIndex(x -> x >= sl_tight_price, arraySlice(fwd_highs, 1, phase1_bars)) AS raw_sl_seg1,
        -- Segment 2: bars phase1_bars+1..max_bars with WIDE SL (give survivors room)
        arrayFirstIndex(x -> x >= sl_wide_price,
    ...
    SELECT
        *,
        ...
        CASE
            ...
            ELSE 0
        END AS raw_sl_bar,
        -- Strategy B: Phase 1 hit = tight SL, Phase 2 hit = wide SL
        CASE
            WHEN raw_sl_seg1 > 0 THEN sl_tight_price
            WHEN raw_sl_seg2_local > 0 THEN sl_wide_price
            ELSE 0
