-- Champion Oracle: Per-trade output for turnover_imbalance_lt_p25__price_impact_lt_p25
-- Inverted barrier (TP=0.25x, SL=0.50x, max_bars=100) — Gen510 optimal
-- Pattern: 2down (2 consecutive DOWN bars) + TI>p95 + kyle>0
-- Features: turnover_imbalance < p25_rolling, price_impact < p25_rolling (signal-relative)
...
signal_detection AS (
    SELECT
        ...
        turnover_imbalance AS feature1_val,
        price_impact AS feature2_val,
...
feature1_with_quantile AS (
    SELECT
        *,
        quantileExactExclusive(0.25)(feature1_val) OVER (
            ORDER BY close_time_ms
            ROWS BETWEEN 999 PRECEDING AND 1 PRECEDING
        ) AS feature1_q
...
feature2_with_quantile AS (
    SELECT
        *,
        quantileExactExclusive(0.25)(feature2_val) OVER (
            ORDER BY close_time_ms
            ROWS BETWEEN 999 PRECEDING AND 1 PRECEDING
        ) AS feature2_q
