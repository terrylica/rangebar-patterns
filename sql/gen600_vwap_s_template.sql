-- ============================================================================
-- Gen 600: Hybrid Feature Sweep — VWAP Dislocation SHORT Template
--
-- PURPOSE: Test hybrid feature pairs (bar-level x lookback/intra) with
-- 3 barrier profiles per query via CROSS JOIN.
-- SHORT mirror of gen600_vwap_l_template.sql: barrier math flipped.
-- NOTE: vcd<p10 is structurally impossible for UP bars (close>VWAP → vcd>0).
--
-- BASE PATTERN: Single UP bar + vwap_close_deviation > p90 → SHORT
-- SHORT ID: vwap_s
--
...
running_stats AS (
    SELECT
        *,
        ...
        quantileExactExclusive(0.90)(vwap_close_deviation) OVER (
            ORDER BY close_time_ms
            ROWS BETWEEN 999 PRECEDING AND 1 PRECEDING
        ) AS vcd_p90_rolling
    FROM base_bars
...
signal_detection AS (
    SELECT
        ...
        lagInFrame(vcd_p90_rolling, 0) OVER w AS vcd_p90_prior,
...
champion_signals AS (
    ...
    WHERE dir_0 = 1
      AND vcd_0 > vcd_p90_prior
      AND rn > 1000
      AND vcd_p90_prior IS NOT NULL
