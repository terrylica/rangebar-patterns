-- ============================================================================
-- Gen 600: Hybrid Feature Sweep — DOWN + Intra Exhaustion (No Gate) LONG Template
--
-- PURPOSE: Test hybrid feature pairs (bar-level x lookback/intra) with
-- 3 barrier profiles per query via CROSS JOIN.
-- Forked from gen600_2down_template.sql: DOWN bar with intra_mdd eligible,
-- NO p75 gate — features replace the gate. Highest coverage (~49%).
--
-- BASE PATTERN: Single DOWN bar + intra_max_drawdown IS NOT NULL → LONG
-- SHORT ID: exh_l_ng
--
