-- ============================================================================
-- Gen 600: Hybrid Feature Sweep — DOWN-UP-UP SHORT Template
--
-- PURPOSE: Test hybrid feature pairs (bar-level x lookback/intra) with
-- 3 barrier profiles per query via CROSS JOIN.
-- SHORT mirror of gen600_udd_template.sql: momentum reversal (D-U-U).
--
-- BASE PATTERN: DOWN-UP-UP + ti>p95 + kyle<0 → SHORT
-- SHORT ID: duu_s
--
...
--   __CONFIG_ID__        — Config identifier (e.g., udd__ofi_gt_p50__lookback_hurst_lt_p50)
--
...
champion_signals AS (
    ...
    WHERE dir_2 = 0 AND dir_1 = 1 AND dir_0 = 1
