-- ============================================================================
-- Gen 600: Hybrid Feature Sweep — Wickless 2-DOWN LONG Template
--
-- PURPOSE: Test hybrid feature pairs (bar-level x lookback/intra) with
-- 3 barrier profiles per query via CROSS JOIN.
-- Forked from gen600_2down_template.sql: both bars have zero opposite wick.
-- Wick < 0.001 tolerance for floating point safety.
--
-- BASE PATTERN: 2 consecutive DOWN bars + both wickless → LONG
-- SHORT ID: wl2d
--
...
--   __CONFIG_ID__        — Config identifier (e.g., wl2d__ofi_gt_p50__lookback_hurst_lt_p50)
--
...
signal_detection AS (
    SELECT
        ...
        opposite_wick_pct AS wick_0,
        lagInFrame(opposite_wick_pct, 1) OVER w AS wick_1,
