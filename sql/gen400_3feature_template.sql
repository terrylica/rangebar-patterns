-- ============================================================================
-- Gen 400: Three-Feature Combination Sweep — Parameterized Template
--
-- PURPOSE: Apply THREE simultaneous microstructure feature filters on top of
-- the champion pattern, with fixed 2:1 R:R barriers (TP=0.5x, SL=0.25x,
-- max_bars=50). Tests whether 3-feature combinations push Kelly positive.
--
...
--   __FEATURE_COL_3__    — Third feature column (e.g., duration_us)
--   __QUANTILE_PCT_1__   — First quantile level (e.g., 0.50)
--   __QUANTILE_PCT_2__   — Second quantile level (e.g., 0.75)
--   __QUANTILE_PCT_3__   — Third quantile level (e.g., 0.50)
...
WITH
-- CTE 1: Base bars — OHLCV + all three feature columns + row numbering
base_bars AS (
    SELECT
        timestamp_ms,
        open, high, low, close,
...
-- CTE 3: Signal detection — lag all three features + entry price
signal_detection AS (
    SELECT
        timestamp_ms,
        open, high, low, close,
...
-- CTE 5: Apply ALL THREE feature filters (AND condition)
signals AS (
    SELECT *
