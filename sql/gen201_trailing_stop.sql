-- ============================================================================
-- Gen 201: Trailing Stop Barrier in ClickHouse SQL
-- GitHub Issue: https://github.com/terrylica/opendeviationbar-patterns/issues/4
-- Copied from: gen200_triple_barrier.sql
--
-- PURPOSE: Replace fixed SL with trailing stop loss that ratchets upward
-- as price moves in our favor. TP barrier + trailing SL + time barrier.
--
-- KEY DIFFERENCE FROM GEN200:
--   Fixed SL (entry_price * (1 - sl_mult * threshold))
--   → Trailing SL (running_max * (1 - trail_mult * threshold))
--   Running max = max(entry_price, max(fwd_highs[1..i])) at each bar i
--   Trailing SL only ratchets UP (never decreases)
--
-- IMPLEMENTATION:
--   arrayMap + arrayReduce for per-bar running max computation
--   arrayFirstIndex with dual-array lambda for trailing SL breach detection
--
-- All Gen200 audit fixes preserved (#1-#9)
-- ============================================================================

-- Tables already created by gen200_triple_barrier.sql
-- Gen201 reuses barrier_results (sl_mult column stores trail_mult)

-- ============================================================================
-- Main query: Trailing stop barrier sweep for SOL @250dbps
-- ============================================================================

-- Clear previous Gen201 results for this threshold
ALTER TABLE opendeviationbar_cache.barrier_results
    DELETE WHERE generation = 201 AND symbol = 'SOLUSDT' AND threshold_decimal_bps = 250;
...
-- Gen201: trail_mult replaces sl_mult (trailing stop instead of fixed SL)
param_expanded AS (
    SELECT
        close_time_ms,
        entry_price,
...
param_with_prices AS (
    SELECT
        ...
        running_maxes,
        -- Trailing SL at each bar position
...
-- TP: arrayFirstIndex on fwd_highs (same as Gen200)
-- Trailing SL: arrayFirstIndex with dual-array lambda (low[i] <= trailing_sl[i])
barrier_scan AS (
    SELECT
        close_time_ms,
        entry_price,
...
-- Gen201: raw_trail_bar replaces raw_sl_bar; trailing SL wins ties
trade_outcomes AS (
    SELECT
        close_time_ms,
        entry_price,
        ...
        max_bars,
        tp_price,
        -- Exit type: trailing SL wins ties (same-bar ambiguity rule)
        CASE
        ...
        CASE
            ...
            ELSE 0
        END AS exit_price,
        -- Trailing SL at exit bar (for telemetry)
        CASE
            WHEN raw_trail_bar > 0 THEN trailing_sls[raw_trail_bar]
            ELSE 0
        END AS trail_sl_at_exit,
        raw_tp_bar,
        raw_trail_bar,
        window_bars
    FROM barrier_scan
...
-- Gen201: TRAIL exits are losses (stopped out); TP + profitable TIME = wins
SELECT
    'SOLUSDT' AS symbol,
    250 AS threshold_decimal_bps,
    'gen201_trailing_stop' AS pattern_name,
    201 AS generation,
    tp_mult,
