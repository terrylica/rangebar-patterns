-- ============================================================================
-- Gen 600: Hybrid Feature Sweep — VWAP Dislocation LONG Template
--
-- PURPOSE: Test hybrid feature pairs (bar-level x lookback/intra) with
-- 3 barrier profiles per query via CROSS JOIN.
-- Forked from gen600_2down_template.sql: extreme negative vcd dislocation.
-- NOTE: vcd>p90 is structurally impossible for DOWN bars (close<VWAP → vcd<0).
--
-- BASE PATTERN: Single DOWN bar + vwap_close_deviation < p10 → LONG
-- SHORT ID: vwap_l
--
...
--   __CONFIG_ID__        — Config identifier (e.g., vwap_l__ofi_gt_p50__lookback_hurst_lt_p50)
--
...
-- DATE CUTOFF: close_time_ms <= 1738713600000 (2026-02-05 00:00:00 UTC)
-- ============================================================================
...
running_stats AS (
    SELECT
        *,
        ...
        quantileExactExclusive(0.10)(vwap_close_deviation) OVER (
            ORDER BY close_time_ms
            ROWS BETWEEN 999 PRECEDING AND 1 PRECEDING
        ) AS vcd_p10_rolling
    FROM base_bars
...
signal_detection AS (
    SELECT
        ...
        vwap_close_deviation AS vcd_0,
        lagInFrame(vcd_p10_rolling, 0) OVER w AS vcd_p10_prior,
...
champion_signals AS (
    ...
    WHERE dir_0 = 0
      AND vcd_0 < vcd_p10_prior
      AND rn > 1000
      AND vcd_p10_prior IS NOT NULL
...
SELECT
    ...
    minIf(close_time_ms, exit_type != 'INCOMPLETE') AS signal_min_ts_ms,
    maxIf(close_time_ms, exit_type != 'INCOMPLETE') AS signal_max_ts_ms,
    -- Total return
