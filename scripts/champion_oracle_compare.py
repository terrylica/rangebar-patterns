"""Champion Oracle: 5-gate trade-by-trade comparison of SQL vs backtesting.py.

Validates the universal champion: turnover_imbalance_lt_p25__price_impact_lt_p25
Pattern: 2down + TI>p95 + kyle>0, Barrier: TP=0.25x, SL=0.50x, max_bars=100

ADR: docs/adr/2026-02-06-repository-creation.md
GitHub Issue: https://github.com/terrylica/opendeviationbar-patterns/issues/26

Usage:
    # Default (universal champion on SOLUSDT@500):
    uv run --python 3.13 python scripts/champion_oracle_compare.py \
        --sql-tsv /tmp/sql_champion_solusdt_500_trades.tsv

    # Override asset:
    uv run --python 3.13 python scripts/champion_oracle_compare.py \
        --sql-tsv /tmp/sql_champion_btcusdt_1000_trades.tsv \
        --symbol BTCUSDT --threshold 1000
...
Copied from: scripts/gen600_oracle_compare.py (Gen600 5-gate oracle)
"""
...
def main():
    parser = argparse.ArgumentParser(description="Champion 5-gate oracle comparison")
    ...
    # Strategy config (defaults match universal champion)
    parser.add_argument("--pattern", default="2down", help="Pattern id")
    parser.add_argument("--feature1", default="turnover_imbalance", help="Feature 1 name")
    parser.add_argument("--dir1", default="lt", help="Feature 1 direction (lt/gt)")
    parser.add_argument("--q1", type=float, default=0.25, help="Feature 1 quantile")
    parser.add_argument("--feature2", default="price_impact", help="Feature 2 name")
    parser.add_argument("--dir2", default="lt", help="Feature 2 direction (lt/gt)")
    parser.add_argument("--q2", type=float, default=0.25, help="Feature 2 quantile")
    parser.add_argument("--tp-mult", type=float, default=0.25, help="TP barrier multiplier")
    parser.add_argument("--sl-mult", type=float, default=0.50, help="SL barrier multiplier")
    parser.add_argument("--max-bars", type=int, default=100, help="Max bars time barrier")
    parser.add_argument("--extra-columns", default="turnover_imbalance,price_impact",
