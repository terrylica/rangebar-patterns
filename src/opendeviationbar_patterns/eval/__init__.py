"""Strategy evaluation metrics: DSR, MinBTL, Omega, CSCV/PBO, E-values, TAMRS, Signal Regularity, Ranking.

GitHub Issues: #12 (eval stack), #16 (TAMRS), #17 (signal regularity + Kelly removal + ranking)

Usage::

    from opendeviationbar_patterns.eval import compute_omega, compute_minbtl
    from opendeviationbar_patterns.eval import compute_rachev, compute_cdar, compute_tamrs
    from opendeviationbar_patterns.eval import compute_signal_regularity
    from opendeviationbar_patterns.eval import percentile_ranks, apply_cutoff, run_ranking_with_cutoffs
    from opendeviationbar_patterns.eval.dsr import compute_psr, expected_max_sr
    from opendeviationbar_patterns.eval.ou_barriers import calibrate_ou, ou_barrier_ratio
"""

from opendeviationbar_patterns.eval.cdar import compute_cdar
from opendeviationbar_patterns.eval.dsr import compute_psr, expected_max_sr, sr_standard_error
from opendeviationbar_patterns.eval.evalues import compute_evalues
from opendeviationbar_patterns.eval.minbtl import compute_minbtl
from opendeviationbar_patterns.eval.omega import compute_omega
from opendeviationbar_patterns.eval.rachev import compute_rachev
from opendeviationbar_patterns.eval.ranking import (
    apply_cutoff,
    knee_detect,
    percentile_ranks,
    run_ranking_with_cutoffs,
)
from opendeviationbar_patterns.eval.signal_regularity import compute_signal_regularity
from opendeviationbar_patterns.eval.tamrs import compute_tamrs

__all__ = [
    "apply_cutoff",
    "compute_cdar",
    "compute_evalues",
    "compute_minbtl",
    "compute_omega",
    "compute_psr",
    "compute_rachev",
    "compute_signal_regularity",
    "compute_tamrs",
    "expected_max_sr",
    "knee_detect",
    "percentile_ranks",
    "run_ranking_with_cutoffs",
    "sr_standard_error",
]
