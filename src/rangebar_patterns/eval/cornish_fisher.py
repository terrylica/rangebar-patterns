...
def cf_expected_shortfall(
    ...
    ES ~ mean - std * phi(z_cf) / Phi(z_cf) adjusted for higher moments.
...
def main():
    ...
    output_file = rd / "cornish_fisher.jsonl"
    ...
    for r in records:
        ...
        if r.get("error") or n_trades < 3:
            results.append({
                ...
                "tail_risk_ratio": None, "mean_over_cf_es_05": None,
        ...
        results.append({
            "config_id": config_id, "n_trades": n_trades,
            "cf_var_01": round(cv01, 8), "cf_var_05": round(cv05, 8),
            "cf_es_01": round(ce01, 8), "cf_es_05": round(ce05, 8),
            "gaussian_var_05": round(gv05, 8), "gaussian_es_05": round(ge05, 8),
