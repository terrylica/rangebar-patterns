# CLAUDE.md - Open Deviation Bar Patterns

Open deviation bar pattern discovery via ClickHouse SQL brute-force analysis of microstructure features.
...
CLAUDE.md (this file)                                    <-- Hub: Navigation + Essentials
    |
    +-- src/opendeviationbar_patterns/eval/CLAUDE.md              <-- Spoke: Eval metrics subpackage
    +-- src/opendeviationbar_patterns/introspect.CLAUDE.md        <-- Spoke: Trade reconstruction tool
...
| Topic             | Document                                                                                                  |
| ----------------- | --------------------------------------------------------------------------------------------------------- |
| Eval Metrics      | [src/opendeviationbar_patterns/eval/CLAUDE.md](/src/opendeviationbar_patterns/eval/CLAUDE.md)             |
| Trade Inspection  | [src/opendeviationbar_patterns/introspect.CLAUDE.md](/src/opendeviationbar_patterns/introspect.CLAUDE.md) |
...
TAMRS replaces Kelly as primary ranker ([Issue #16](https://github.com/terrylica/opendeviationbar-patterns/issues/16)).
...
Decision: [Issue #12](https://github.com/terrylica/opendeviationbar-patterns/issues/12) | [Issue #16](https://github.com/terrylica/opendeviationbar-patterns/issues/16) | Code: `src/opendeviationbar_patterns/eval/`
...
| Inspect trade      | `OPENDEVIATIONBAR_INSPECT_CONFIG_ID=... mise run trade:inspect` |
...
- Time column: `close_time_ms` (renamed from `timestamp_ms`)
- Column: `threshold_decimal_bps` (NOT threshold_dbps)
- Ouroboros: `AND ouroboros_mode = 'month'` required in all WHERE clauses (year bars deleted upstream)
- Remote host: Set `OPENDEVIATIONBAR_CH_HOST` in `.mise.local.toml`
...
| File                                                | Purpose                                        |
| --------------------------------------------------- | ---------------------------------------------- |
| `src/opendeviationbar_patterns/champion.py`         | Champion pattern constants (SSoT)              |
...
| `src/opendeviationbar_patterns/config.py`           | Typed env reader for OPENDEVIATIONBAR\_\* vars |
| `src/opendeviationbar_patterns/introspect.py`       | Atomic trade reconstruction (Issue #13)        |
| `src/opendeviationbar_patterns/eval/`               | TAMRS eval subpackage (Issue #12, #16)         |
| `src/opendeviationbar_patterns/eval/tamrs.py`       | TAMRS composite score                          |
| `src/opendeviationbar_patterns/eval/rachev.py`      | Rachev ratio (tail asymmetry)                  |
| `src/opendeviationbar_patterns/eval/cdar.py`        | CDaR (clustered loss detection)                |
| `src/opendeviationbar_patterns/eval/ou_barriers.py` | OU calibration + barrier ratio                 |
