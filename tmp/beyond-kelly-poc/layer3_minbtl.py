"""Agent 4: Minimum Backtest Length (MinBTL) gate.

Computes the minimum number of trades needed for statistical significance
given the number of trials tested (N=1008). Configs with fewer trades than
MinBTL are flagged as data-insufficient.
...
OUTPUT_FILE = RESULTS_DIR / "minbtl_gate.jsonl"
...
def compute_minbtl(sr: float, n_trials: int, skew: float, kurt: float) -> float:
    ...
    MinBTL ≈ (2 * log(N) / SR²) * (1 - skew*SR/3 + ((kurt-1)/4)*SR²)
    ...
    moment_adj = max(moment_adj, 0.1)  # Guard against negative
